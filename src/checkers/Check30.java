 ...
 * 
 * Checker class for question #30
 * 
 * NOTE: The version uploaded before had bugs, this is the debugged version with comments included
 */

public class Check30 extends Check
{
    //right method for question:
    //Return false if an array contains any consecutive number.
...
//check method looking to test cases for comparing users' methods' output with the right method
    public static boolean check() throws Exception
    {
        ...
        Method m = c.getMethod("isConsecutive", int[].class);
        int[] x = {2,3,4,5,6,7,8};
        if ( isConsecutive(x) == (boolean)m.invoke(null, x) )
            return true;
        else
            return false;
    }
